...
# ==== Config ====
m = [round(x, 3) for x in np.linspace(0.01, 0.3, 50)]
n = [round(x, 2) for x in np.linspace(0, 360, 10)]
o = [round(x, 2) for x in np.linspace(-0.5, 0.5, 10)]

ram_threshold_percent = 90
disk_check_interval = 100
critical_disk_usage_percent = 90
CHUNK_SIZE = 5000  # Save to new CSV every 10,000 iterations

model_output_dir = '/Volumes/T7 Shield/Sim 16'
        ...
        for i in range(start_i, len(m)):
            ...
            for j in range(j_start_index, len(n)):
                ...
                for k in range(k_start_index, len(o)):
                    ...
                    glafic.set_lens(1, 'pow', 0.261343256161012, 1.0, 0.0, 0.0, 0.21, -1.39, 0.46, 2.1)
