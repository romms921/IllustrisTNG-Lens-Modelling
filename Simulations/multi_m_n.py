...
# ==== Config ====
# --- Simulation Parameters (2 Variables) ---
...
restart_file_path = os.path.join(os.path.dirname(log_file_path), 'simulation_restart_state_2var.json')
...
def load_restart_state(path):
    ...
    try:
        with open(path, 'r') as f:
            state = json.load(f)
            i, j, chunk = state.get('i', 0), state.get('j', 0), state.get('chunk_number', 1)
            with open(sys.__stdout__.fileno(), 'w', closefd=False) as real_stdout:
...
def run_single_model(params):
    i, j, m_val, n_val = params
    model_name = f"glafic_run_{uuid.uuid4()}"
    model_path = os.path.join(model_output_dir, model_name)
    try:
        glafic.init(0.3, 0.7, -1.0, 0.7, model_path, -3.0, -3.0, 3.0, 3.0, 0.01, 0.01, 1, verb=0); glafic.set_secondary('chi2_splane 1', verb=0); glafic.set_secondary('chi2_checknimg 0', verb=0); glafic.set_secondary('chi2_restart   -1', verb=0); glafic.set_secondary('chi2_usemag    1', verb=0); glafic.set_secondary('hvary          0', verb=0); glafic.set_secondary('ran_seed -122000', verb=0); glafic.startup_setnum(2, 0, 1); glafic.set_lens(1, 'sie', 0.261343256161012, 1.563051e+02, 0.0, 0.0, 2.168966e-01, -1.398259e+00,  0.0, 0.0); glafic.set_lens(2, 'pert', 0.261343256161012, 1.0, 0.0, 0.0, m_val, n_val, 0.0, 0.0); glafic.set_point(1, 1.0, 0.0, 0.0); glafic.setopt_lens(1, 0, 0, 1, 1, 1, 1, 0, 0); glafic.setopt_lens(2, 0, 0, 0, 0, 1, 1, 0, 0); glafic.setopt_point(1, 0, 1, 1); glafic.model_init(verb=0); glafic.readobs_point('/Users/ainsleylewis/Documents/Astronomy/IllustrisTNG Lens Modelling/System 2/pos_point.dat'); glafic.optimize(); glafic.findimg(); glafic.quit()
        ...
        result_dict = {'strength': m_val, 'pa': n_val, 'num_images': num_images, 'pos_rms': pos_rms, 'mag_rms': mag_rms, 't_mpole_str': dfs[1]['$\epsilon$'].iloc[0] if dfs and len(dfs) > 1 else 0, 't_mpole_pa': dfs[1]['$θ_{m}$'].iloc[0] if dfs and len(dfs) > 1 else 0, 'chi2': chi2, **{col: dfs[0][col].iloc[0] if dfs else 0 for col in macro_columns}}
    ...
    try:
        with multiprocessing.Pool(processes=NUM_PROCESSORS) as pool:
            ...
            for i_res, j_res, result_dict in results_iterator:
                ...
                # Calculate stats for the log dictionary
                percentage_complete = (iterations_done / total_iterations) * 100
                ...
                # Assemble the dictionary in the requested format
                progress_info = {
