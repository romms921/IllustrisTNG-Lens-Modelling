...
model_output_dir = '/Volumes/LinuxTechTi/Astro/Sim 3'
    ...
    for i in range(len(m)):
        for j in range(len(n)):
            for k in range(len(o)):
                ...
                glafic.readobs_point('/Modelling/obs_point/obs_point_(POS).dat')
                glafic.optimize()
                glafic.findimg()
                ...
                # ---- RAM Monitoring ----
                if get_memory_usage() > ram_threshold_percent:
                ...
                if iteration_count % disk_check_interval == 0:
                    ...
                    threat = get_disk_threat_assessment(model_output_dir, avg_model_size, models_remaining)
                    total, used, free = shutil.disk_usage(model_output_dir)
                    disk_used_percent = 100 * used / total

                    print(f"🧠 Disk Threat: {threat} | Used: {disk_used_percent:.2f}%")

                    with open(log_file_path, 'a') as log_file:
                        log_file.write(f"[{iteration_count}] Disk Threat: {threat} | Used: {disk_used_percent:.2f}%\n")

                    if iteration_count == disk_check_interval and threat == "HIGH":
                        print("⚠️ Initial disk overflow risk high. Continuing cautiously.")

                    if threat == "HIGH" and disk_used_percent >= critical_disk_usage_percent:
